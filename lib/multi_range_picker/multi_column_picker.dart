...
import 'multi_column_converter.dart';
import 'multi_column_list.dart';
...
class BrnMultiColumnPicker extends StatefulWidget {
  ...
  final OnSelectEntityInterceptor? canSelectEntryInterceptor;
...
class _BrnSelectionGroupViewState extends State<BrnMultiColumnPicker> {
  ...
  int get _columnCount {
    return MultiColumnPickerUtil.getTotalColumnCount(widget.entity);
  }
  ...
  Widget _listWidget() {
    ...
    if (!PhoenixTools.isEmpty(_firstList) &&
      ...
      /// 1、仅有一级的情况
      /// 1.1 一级单选 && 没有自定义范围的情况
      widgets.add(MultiColumnListWidget(
          items: _firstList,
