...
enum ColumnDateRangeType { year, month, day }

/// one side widget of DateRangePicker
// ignore: must_be_immutable
class DateRangeSideWidget extends StatefulWidget {
  /// 可选最小时间
  ...
  State<StatefulWidget> createState() => _DatePickerWidgetState(
      minDateTime, maxDateTime, initialStartDateTime, onInitSelectChange);
}

class _DatePickerWidgetState extends State<DateRangeSideWidget> {
      ...
      child: Container(
        ...
        child: Picker.builder(
          ...
          itemBuilder: (context, index) => _renderDatePickerItemComponent(
              format.contains("y")
                  ? ColumnDateRangeType.year
                  : (format.contains("M")
                      ? ColumnDateRangeType.month
  ...
  Widget _renderDatePickerItemComponent(
      ColumnDateRangeType columnType, int index, int value, String format) {
    TextStyle textStyle = widget.themeData!.itemTextStyle.generateTextStyle();
    if ((ColumnDateRangeType.year == columnType &&
            index == _calcSelectIndexList()[0]) ||
        (ColumnDateRangeType.month == columnType &&
            index == _calcSelectIndexList()[1]) ||
        (ColumnDateRangeType.day == columnType &&
